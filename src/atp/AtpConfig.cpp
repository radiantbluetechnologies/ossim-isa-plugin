...
#include <isa/atp/AtpOpenCV.h>
#include <isa/atp/descriptor/Descriptor.h>
#include <isa/atp/descriptor/Matcher.h>
...
bool AtpConfig::readConfig(const string& cn)
{
   ...
   try
   {
      ...
      if (!shareDir.isDir())
      {
         // Try the development (non-install) configuration:
         shareDir = ossimPreferences::instance()->
               preferencesKWL().findKey( std::string( "ossim_isa_share_directory" ) );
         if (!shareDir.isDir())
