...
int main(int argc, char* argv[])
{
   ...
   argumentParser.getApplicationUsage()->setDescription(argumentParser.getApplicationName() + " takes an input image and generates a corresponding RPC geometry.");
   ...
   argumentParser.getApplicationUsage()->addCommandLineOption("--geom","Default -- Outputs the RPC to an OSSIM geometry file. The input file's base name is used.");
   argumentParser.getApplicationUsage()->addCommandLineOption("--json","Outputs the RPC to a JSON file. The input file's base name is used.");
   argumentParser.getApplicationUsage()->addCommandLineOption("--rpb","Output WorldView-style RPB format. The input file's base name is used.");
   argumentParser.getApplicationUsage()->addCommandLineOption("--tolerance <double>","Used as an RMS error tolerance in meters between original model and RPC.");
   argumentParser.getApplicationUsage()->addCommandLineOption("--xml","Output XML-formatted RPC. The input file's base name is used.");
...
#else
   {
      ossimNotify(ossimNotifyLevel_FATAL) << "ERROR: JSON output requested but JSON is not available in this build! <"<< std::endl;
      exit(1);
   }
#endif
...
//   ISA::Image* image = new ISA::Image("XXX", file);
//   std::vector< std::pair<std::string, std::string> > availableModels;
//   image->getAvailableModels(availableModels);
//   for (size_t i=0; i<availableModels.size(); ++i)
//   {
//      Json::Value arrayEntry;
//      cout<<"plugin: "<<availableModels[i].first<<endl;
//      cout<<"model: "<<availableModels[i].second<<endl<<endl;
//   }
   ...
   bool converged = solver->solveCoefficients(geom.get(), error);
